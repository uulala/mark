<template>
  <view class="content">
    ...
    <canvas
      ...
      @touchend="touchend"
    ></canvas>
    <image class="page-btn pre" :src="pageHistory.length < 2 ? '/static/icon/ar-left3.png': '/static/icon/ar-left2.png'" @click="toPre"></image>
    ...
    <!-- <view class="page-box">
      <view class="btn s-btn" @click="addPage">新增空白页</view>
    </view> -->
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    touchend() {
      this.ctx.closePath();
      const line = { color: this.ctx.strokeStyle, linePoints: this.linePoints };
...
.content{
  position: relative;
  height: 100vh;
  /* background: linear-gradient(to bottom right,#7bdcb5, white,  #8ed1fc); */
}
.tool-wrapper{
  display: flex;
...
.btn {
  ...
  border-radius: 5px;
  padding: 3px 5px;
  background: linear-gradient(to bottom right,white, #8ed1fc);
}
.draft {
  width: 100vw;
